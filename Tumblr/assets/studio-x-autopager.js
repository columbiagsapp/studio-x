...
var tumblrAutoPager = {
    ...
    init: function () {
		...
		function truncateAutopager() {
			console.log('calling autopager');
		 // $('.caption').truncate({max_length: MAX_CAPTION_LENGTH});
            ...
            for (var i = 0; i < posts.length; i++) {
            	switch(i%3){
            		case 1:
            			...
            			//var ht = String(posts[i].outerHTML);
            			
            			
            			
            			//console.log( ht );//.truncate({max_length: 400});
            			
            			//console.log(posts[i].getElementsByTagName('caption'));
            			//$(posts[i]).find(".caption").truncate({max_length: 400});
            			break;
            		default:
            			break;
            	}
